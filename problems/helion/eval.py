...
# These modules live in the problem directory (set up via sys.path at runtime),
# so we import them lazily inside functions rather than at module level.
# - reference: check_implementation, generate_input
# - utils: set_seed, clear_l2_cache
# - task: TestSpec
...
def _do_bench_cudagraph(fn, rep_ms=100, return_mode="mean", clear_l2=True):
    ...
    with torch.cuda.stream(torch.cuda.Stream()):
        ...
        if clear_l2:
            ret = [t - c for t, c in zip(total_times, cache_clear_times)]
        else:
            ret = total_times
...
def run_local():
    ...
    if mode in ("benchmark", "both"):
        ...
        for idx, bench in enumerate(benchmarks):
            ...
            if isinstance(result, Stats):
                median_ms = result.mean / 1e6  # Stats stores ns
                min_ms = result.best / 1e6
                max_ms = result.worst / 1e6
                print(f"  Benchmark {idx}: {median_ms:.4f} ms (min={min_ms:.4f}, max={max_ms:.4f})  {bench.spec}")
            else:
